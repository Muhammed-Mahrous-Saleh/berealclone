...
function generateFakeUsers(count: number = 10) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        // Limit user_name to reasonable length (first 8 chars of first+last + 8 char ID = max 16 chars)
        const namePart = `${firstName.toLowerCase().slice(0, 4)}${lastName.toLowerCase().slice(0, 4)}`;
        const user_name = `${namePart}${shortId}`;
...
async function seed() {
    ...
    try {
        ...
        for (let i = 0; i < fakeUsers.length; i++) {
            ...
            console.log(
                `Creating user ${i + 1}/10: ${fakeUser.name} (${fakeUser.user_name})...`,
            );
            ...
            try {
                ...
                if (userExists) {
                    ...
                    if (authError) {
                        ...
                        if (maybeCreated) {
                            ...
                            console.error(
                                `  💡 The trigger probably tries to INSERT into profiles with a NULL user_name.`,
                            );
                            console.error(
                                `  💡 Solution: Modify your database trigger to handle NULL user_names or provide a default value.\n`,
                            );
                            continue;
                        }
                    } else {
                    ...
                    // Immediately create/update profile with user_name
                ...
                if (existingPost) {
                    console.log(
                        `  ⚠️  User already has an active post, skipping...`,
                    );
                } else {
